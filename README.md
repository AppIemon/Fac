...
| `cobblegen_tnt` | **TNT 복제 조약돌 생성기** (참고 설계 원본) | 완전 자동 |
| `kelpfarm_bonemeal` | **뼛가루 켈프 팜** — 뼛가루 1개 = 켈프 1개, 한 줄이면 충분 | 완전 자동 |
| `bubble_lift` | 물기둥 아이템 엘리베이터 (되먹임 고리용) | 완전 자동 |
| `full_factory` | **통합 공장(압축판)** — TNT 조약돌 + 2단 제련 + 켈프 연료 + 이끼 뼛가루 자급. 16x39x11 | TNT 생성기만 별도 |

이전에 있던 `smoothstone_factory` / `cobble_factory` 는 `full_factory` 가 대체해서 지웠다.
TNT 복제가 막힌 서버라면 `cobblegen`(물먹임 계단 조약돌 생성기)을 조약돌 투입 상자에
물리면 된다 — 나머지는 그대로다.
